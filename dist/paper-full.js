...
(function(process) {
  var paper = new function(undefined) {
      ...
      return {
        ...
        solveQuadratic: function(a, b, c, roots, min, max) {
          ...
          if (isFinite(x1) && (min == null || x1 >= min && x1 <= max))
            roots[count++] = x1;
          if (x2 !== x1 && isFinite(x2) && (min == null || x2 >= min && x2 <= max))
            roots[count++] = x2;
          return count;
        },
        solveCubic: function(a, b, c, d, roots, min, max) {
          ...
          if (isFinite(x) && (count === 0 || x !== roots[count - 1]) && (min == null || x >= min && x <= max))
            roots[count++] = x;
          return count;
        }
      };
    ...
    var Item = Base.extend(Emitter, {
      ...
      _hitTest: function(point, options) {
        ...
        var matrix = this._matrix,
            ...
            tolerancePadding = options._tolerancePadding = new Size(Path._getPenPadding(1, totalMatrix.inverted())).multiply(Math.max(options.tolerance, 0.000001));
    ...
    var Segment = Base.extend({
      ...
      _serialize: function(options) {
        return Base.serialize(this.isStraight() ? this._point : [this._point, this._handleIn, this._handleOut], options, true);
      },
      ...
      isCollinear: function(segment) {
        return Segment.isCollinear(this, this.getNext(), segment, segment.getNext());
      },
      isColinear: '#isCollinear',
      ...
      statics: {
        isLinear: function(seg1, seg2) {
          ...
          return l.isCollinear(seg1._handleOut) && l.isCollinear(seg2._handleIn);
        },
    ...
    var Curve = Base.extend({
      ...
      isCollinear: function(curve) {
        return Ssegment.isCollinear(this._segment1, this._segment2, curve._segment1, curve._segment2);
      },
      isOrthogonalArc: function() {
      ...
      statics: {
        ...
        isLinear: function(v) {
          ...
          return l.isCollinear(new Point(v[2] - p1x, v[3] - p1y)) && l.isCollinear(new Point(v[4] - p2x, v[5] - p2y));
        },
      ...
      function evaluate(v, t, type, normalized) {
        ...
        if (type === 0 && (t < tolerance || t > 1 - tolerance)) {
          ...
          if (type === 0) {
            ...
            if (normalized) {
              if (x === 0 && y === 0 && (t < tolerance || t > 1 - tolerance)) {
                x = c2x - c1x;
                y = c2y - c1y;
              }
      ...
      function addCurveIntersections(v1, v2, curve1, curve2, locations, include, tMin, tMax, uMin, uMax, oldTDiff, reverse, recursion) {
        ...
        if (oldTDiff > 0.5 && tDiff > 0.5) {
          ...
          var t1 = tMinNew + (tMaxNew - tMinNew) / 2,
              t2 = uMin + (uMax - uMin) / 2;
          if (reverse) {
            addLocation(locations, include, curve2, t2, Curve.getPoint(v2, t2), curve1, t1, Curve.getPoint(v1, t1));
          } else {
            addLocation(locations, include, curve1, t1, Curve.getPoint(v1, t1), curve2, t2, Curve.getPoint(v2, t2));
          }
        } else if (tDiff > 0) {
    ...
    var PathItem = Item.extend({
      ...
      getIntersections: function(path, _matrix, _expand) {
        ...
        var locations = [],
            ...
            matrix2 = path ? (_matrix || path._matrix).orNullIfIdentity() : matrix1,
    ...
    var Path = PathItem.extend({
      ...
      statics: {
        isClockwise: function(segments) {
          ...
          for (var i = 0,
              l = segments.length; i < l; i++)
            sum += Curve.getEdgeSum(Curve.getValues(segments[i], segments[i + 1 < l ? i + 1 : 0]));
          return sum > 0;
        },
    ...
    PathItem.inject(new function() {
            ...
            for (var k = 0,
              ...
              if (length <= curveLength) {
                ...
                var curve = node.segment.getCurve(),
                    ...
                    hor = curve.isLinear() && Math.abs(curve.getTangentAt(0.5, true).y) < tolerance,
                    path = curve._path;
      ...
      function splitPath(intersections) {
        ...
        function resetLinear() {
          for (var i = 0,
              l = linearHandles.length; i < l; i++)
            linearHandles[i].set(0, 0);
        }
        for (var i = intersections.length - 1,
            curve,
            prev; i >= 0; i--) {
            ...
            if (winding && (winding === 1 && py >= values[1] && py <= values[7] || py >= values[7] && py <= values[1]) && Curve.solveCubic(values, 1, py, roots, 0, 1) === 1) {
              ...
              if (!(t > tMax && startCounted && curve.next !== curves[i + 1] || t < tMin && prevT > tMax && curve.previous === prevCurve)) {
                var x = Curve.getPoint(values, t).x,
                    ...
                    counted = false;
                if (Numerical.isZero(slope) && !Curve.isLinear(values) || t < tMin && slope * Curve.getTangent(curve.previous.values, 1).y < 0 || t > tMax && slope * Curve.getTangent(curve.next.values, 0).y < 0) {
      ...
      function tracePaths(segments, operator, selfOp) {
        ...
        for (var i = 0,
          ...
          if (path._segments.length > (path._closed ? path.isLinear() ? 2 : 0 : 1))
            paths.push(path);
        }
        return paths;
      }
    ...
    var View = Base.extend(Emitter, {
      ...
      function handleMouseMove(view, point, event) {
        ...
        if (tool) {
          tool._handleEvent(view.dragging && tool.responds('mousedrag') ? 'mousedrag' : 'mousemove', point, event);
        }
        view.update();
        return tool;
      }
      ...
      docEvents[mousemove] = function(event) {
        ...
        if (view) {
          ...
          if (view.dragging || view.getBounds().contains(point))
